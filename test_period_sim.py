...
per=Period_error(0,2,3)
a, b, fr, T = 10, 1, 1, 1000
bintime = (1/fr)/64
num = per.sim_num
nharm = per.nharm
...
for shape in shapes:
    ...
    for i in range(num): 
        ...
        per.guess_freq_array(z_res=500, factor=1/2)
        k0=0
        harm = []
...
for shape in shapes:
    ...
    for sim in range(num):
        ...
        for n in range(num_harm):
            ...
            if plotter >0:
                ...
                ppf.pretty_fit_plot(
                    ax1, freq, harm_data, par_hws, func='sinc', window=0.25*T_obs, skip_data=0)
                ppf.pretty_fit_plot(
                    ax2, freq, zndata, par_ts, func='sinc', window=0.25*T_obs, skip_data=0)
                ppf.pretty_fit_plot(
                    ax2, freq, zndata, par_tg, func='gauss', window=0.25*T_obs, skip_data=1)
                ppf.pretty_fit_plot(
    ...
    sim_path = 'PE_{1:s}{0:s}.npy'.format(shape, path0)
